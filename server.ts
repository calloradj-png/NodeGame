...
async function startServer() {
  ...
  // Rooms store (initially empty, dynamically created)
  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    ws.on("message", (rawMessage: string) => {
      try {
        ...
        if (type === "join") {
          let { name, color, avatarStyle, requestedRoomId, particleTrail, nameEffect, decorFrame, friendsList } = payload;
          ...
          if (targetRoomId) {
            ...
            if (!rooms[targetRoomId]) {
              rooms[targetRoomId] = {
                id: targetRoomId,
                name: getRoomPrettyName(targetRoomId),
                players: {},
                collectibles: []
              };
            }
          } else {
            // Dynamic Matchmaking: Find any room with capacity < 5, else create one
            // Critically EXCLUDE private (private- prefix) and custom-mode rooms!
            // Prefer rooms that have our friends on it
            let bestRoomId = "";
            let maxFriendsInRoom = -1;

            for (const rId of Object.keys(rooms)) {
              const room = rooms[rId];
              const isPrivate = rId.startsWith("private-") || !!(room as any).mode || !!(room as any).password;
              if (!isPrivate) {
                ...
                if (currentCount < 5) {
                  ...
                  if (friendsCount > maxFriendsInRoom) {
                    maxFriendsInRoom = friendsCount;
                    bestRoomId = rId;
                  }
                }
              }
          ...
          // Check if target room is private/friends-only
          ...
          if (Object.keys(oldRoom.players).length === 0) {
            console.log(`Room ${oldRoomId} is empty. Deleting dynamically.`);
          ...
          if (!rooms[newRoomId]) {
            rooms[newRoomId] = {
              id: newRoomId,
              name: getRoomPrettyName(newRoomId),
              players: {},
              collectibles: []
            };
          }
      ...
      if (session) {
        ...
        if (rooms[roomId] && rooms[roomId].players[playerId]) {
          ...
          if (rooms[roomId] && Object.keys(rooms[roomId].players).length === 0) {
            console.log(`Room ${roomId} is empty. Deleting dynamically.`);
            delete rooms[roomId];
          }
        }
  ...
  app.post("/api/purge-cache", async (req, res) => {
    try {
      ...
      console.log(`[ADMIN COMMAND] 'purge_cache' executed. Results:`, results);
